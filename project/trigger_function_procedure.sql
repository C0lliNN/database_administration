...
-- A plataforma está tendo uma grande despesa para armazenar os dados. Como a tabela de avalicao 
-- está muito grande e apenas a média das avaliações para uma determinada sessão é útil, 
-- é necessário realizar a compilação dessa tabela da seguinte maneira:
-- para cada sessao, obtenha a media das avaliações, 
-- remove todas as avaliações para a sessao
-- insera um registro com a media da sesao
...
BEGIN
  ...
  OPEN curs;
  REPEAT
    FETCH curs INTO id_sessao;
    SELECT ROUND(AVG(nota)) INTO media FROM avaliacao WHERE id_sessao = id_sessao;

    DELETE FROM avaliacao WHERE id_sessao = id_sessao;
    INSERT INTO avaliacao VALUES(null, id_sessao, 1, media);
...
CREATE FUNCTION obter_media (id_sessao INT)
RETURNS INT
BEGIN
  DECLARE media INT;
  SELECT ROUND(AVG(nota)) INTO media FROM avaliacao WHERE id_sessao = id_sessao;
